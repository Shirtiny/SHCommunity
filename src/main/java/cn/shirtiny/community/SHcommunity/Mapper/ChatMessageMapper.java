...
public interface ChatMessageMapper extends BaseMapper<ChatMessage> {
    ...
    //查询某个聊天记录的全部消息 ,比如查询聊天室的消息（chatHistoryId为0的消息） 创建时间顺序
    ...
    //分页查询某个聊天记录的消息 ,比如查询聊天室的消息（chatHistoryId为0的消息） 创建时间顺序
    @Select("select * from chat_message where chat_history_id=#{chatHistoryId} ORDER BY gmt_created ASC LIMIT #{offset},#{size}")
    List<ChatMessageDTO> selectAllDTOByhistoryId(@Param("chatHistoryId") String chatHistoryId, @Param("offset") int offset, @Param("size") int size);
