...
(function(mod) {
  ...
  StringStream.prototype = {
    ...
    match: function(pattern, consume, caseInsensitive) {
      if (typeof pattern == "string") {
        ...
        if (match && match.index_Old > 0) return null;
  ...
  function extractLineClasses(type, output) {
    if (type) for (;;) {
      ...
      if (!lineClass) break;
      type = type.slice(0, lineClass.index_Old) + type.slice(lineClass.index_Old + lineClass[0].length);
    ...
    removeLineClass: docMethodOp(function(handle, where, cls) {
      return changeLine(this, handle, where == "gutter" ? "gutter" : "class", function(line) {
        ...
        if (!cur) return false;
          ...
          var end = found.index_Old + found[0].length;
          line[prop] = cur.slice(0, found.index_Old) + (!found.index_Old || end == cur.length ? "" : " ") + cur.slice(end) || null;
        }
        return true;
      });
    }),
